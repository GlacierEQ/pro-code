...
def check_profile(
    root: Path, expected_repo: str, expected_role: str, errors: list[str]
) -> dict[str, Any]:
    profile = load_json(root / ".apex/repo-profile.json", errors)
    expect(
        profile.get("repo") == expected_repo,
        errors,
        f"repo profile mismatch: expected {expected_repo}, got {profile.get('repo')}",
    )
    expect(
        profile.get("helix_role") == expected_role,
        errors,
        f"helix role mismatch: expected {expected_role}, got {profile.get('helix_role')}",
    )
    expect(
        isinstance(profile.get("hidden_substrate"), dict),
        errors,
        "repo profile hidden_substrate must be an object",
    )
    expect(
        "no_raw_secrets" in str(profile.get("secret_policy", "")),
        errors,
        "secret_policy must include no_raw_secrets",
    )
    expect(bool(profile.get("mission")), errors, "repo profile mission is required")
    if expected_role == "doctrine_strand":
        expect(
            bool(profile.get("paired_runtime_strand")),
            errors,
            "doctrine profile must declare paired_runtime_strand",
        )
    if expected_role == "runtime_strand":
        expect(
            bool(profile.get("paired_doctrine_strand")),
            errors,
            "runtime profile must declare paired_doctrine_strand",
        )
    return profile
...
def check_strand(
    ...
    expect(
        strand.get("strand_type") == expected_strand,
        errors,
        f"strand_type mismatch: expected {expected_strand}, got {strand.get('strand_type')}",
    )
    expect(
        isinstance(strand.get("authority"), dict),
        errors,
        "strand authority must be an object",
    )
    expect(
        isinstance(strand.get("non_goals"), list) and bool(strand.get("non_goals")),
        errors,
        "strand non_goals must be a non-empty list",
    )
    paired = strand.get("paired_strand")
    expect(
        isinstance(paired, str) and "/" in paired,
        errors,
        "strand must declare paired_strand as owner/repo",
    )
    if expected_role == "doctrine_strand":
        expect(
            strand.get("authority", {}).get("runtime_execution") is False,
            errors,
            "doctrine strand must not claim runtime_execution authority",
        )
    if expected_role == "runtime_strand":
        expect(
            strand.get("authority", {}).get("runtime_execution") is True,
            errors,
            "runtime strand must claim runtime_execution authority",
        )
    return strand
...
def check_pair(
    ...
    profile_pair = (
        profile.get("paired_runtime_strand")
        if expected_role == "doctrine_strand"
        else profile.get("paired_doctrine_strand")
    )
    expect(
        strand.get("paired_strand") == profile_pair,
        errors,
        f"profile/strand pair mismatch: {profile_pair} vs {strand.get('paired_strand')}",
    )
...
def check_peer(
    ...
    expect(
        strand.get("paired_strand") == peer_repo,
        errors,
        f"local paired_strand does not match peer repo: {strand.get('paired_strand')} vs {peer_repo}",
    )
    expect(
        peer_strand.get("paired_strand") == expected_repo,
        errors,
        f"peer does not point back to {expected_repo}: {peer_strand.get('paired_strand')}",
    )
...
def check_json_schemas(root: Path, errors: list[str]) -> None:
    for rel in [
        ...
        expect(
            schema.get("$schema") is not None, errors, f"schema missing $schema: {rel}"
        )
        expect(
            isinstance(schema.get("required"), list),
            errors,
            f"schema missing required array: {rel}",
        )
    ...
    for path in sorted(files):
        ...
        for regex in SECRET_PATTERNS:
            if regex.search(text):
                errors.append(
                    f"possible raw secret in {path.relative_to(root)} matching {regex.pattern}"
                )
                break


def write_report(
    path: Path, expected_repo: str, expected_role: str, errors: list[str]
) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify Helix substrate identity, drift, and pair integrity."
    )
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--expected-repo", required=True)
    parser.add_argument(
        "--expected-role", choices=sorted(ROLE_TO_STRAND), required=True
    )
    parser.add_argument(
        "--peer-root",
        default=None,
        help="Optional checked-out paired repo root for reciprocal verification.",
    )
